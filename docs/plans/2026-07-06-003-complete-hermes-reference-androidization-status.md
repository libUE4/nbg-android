...
- Model Provider Profiles v4: `NbgModelProviderProfile` registers OpenRouter/OpenAI/Gemini/Qwen/Kimi/Nous/Ollama style provider metadata, supports local import/export plus visual add/edit/delete of user ProviderProfile templates without API keys, and routes model fetch, verification, read-only generation, Hanako URL API provider config, auth header, thinking/extra-body, and chat/model endpoints through provider profiles.
...
| ProviderProfile model plugins | Android `NbgModelProviderProfile` registry, local template import/export, visual template editor, and true provider-profile routing for endpoint/auth/extra-body/model verification/Hanako URL API config | Implemented v4 |
